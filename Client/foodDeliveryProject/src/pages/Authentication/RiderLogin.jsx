...
function RiderLogin() {
  ...
  const handleLogin = (e) => {
    ...
    e.target.reset();
    axios.post(`${url}/rider/login`, data).then((response) => {
      if (!response.data.error) {
        localStorage.setItem("riderAccessToken", response.data);
        setError(true)
  ...
  return (
    ...
    >
      ...
      <div className='absolute inset-0 backdrop-brightness-50 backdrop-blur-sm flex items-center justify-center'>
        <div className='bg-white w-[90%] md:w-[70%] lg:w-[50%] max-w-md md:max-w-lg lg:max-w-xl rounded-xl flex flex-col gap-5 justify-start items-center p-5 md:p-10'>
          <p className='font-bold text-2xl md:text-3xl text-indigo-950'>Rider Login</p>
          <form onSubmit={handleLogin} className='flex flex-col gap-5 w-full'>
            <div className='flex flex-col gap-3 px-5 w-full'>
              <label className='font-bold text-lg md:text-xl' htmlFor="username">Enter rider username</label>
              <input
            ...
            <div className='flex flex-col gap-3 px-5 w-full'>
              <label className='font-bold text-lg md:text-xl' htmlFor="password">Enter rider pin</label>
              <input
                className='border border-1 border-black px-4 py-2 rounded-full w-full'
                type="password"
                id="password"
